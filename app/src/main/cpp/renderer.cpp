...
static const char* VERTEX_SHADER =
    ...
    "out vec4 vColor;\n"
    "float capsule(float x, float a, bool left){\n"
    "   float u = a - (left ? min(x,a) : max(x,a));\n"
    "   return sqrt(1.0 - u*u);\n"
    "}\n"
    "void main() {\n"
...
"        float radius = 0.8 * exp( slope * (offset - time) + thickness * flipflop );\n"
    ...
    "    vec3 color = mix(color_start, color_end, progress);"
    "    color *= color;\n"
    "    vec3 blank = mix(vec3(1), color, 0.2);\n"
    "    vec3 gray = mix(vec3(0.7), color, 0.1);\n"
    "    vColor = vec4( mix(blank, color, index_odd), 1.);\n"
    "}\n";
